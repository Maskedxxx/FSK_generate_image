...
Самодостаточный скрипт — креды S3 из .env, зависит только от boto3.

Структура S3 для каждой задачи:
    {S3_PREFIX}/{task_id}/
    ├── meta.json                          # метаданные задачи (стиль, ответы опросника)
    ├── L1_crops/
    │   ├── schema_x2.png                  # увеличенная планировка
    │   ├── analysis.json                  # результат анализа (комнаты, полигоны)
    │   ├── L1_meta.json                   # мета слоя 1 (модель, время)
    │   ├── schema_with_polygons.png       # планировка с нарисованными полигонами
    │   └── {N}_{room_name}_crop.png       # кроп каждой комнаты (× кол-во комнат)
    ├── L2_references/
    │   └── {room_name}.png                # AI-референс интерьера (× кол-во комнат)
    ├── L3_renders/
    │   ├── {room_name}_prepared_a.png     # подготовленный ракурс A (× кол-во комнат)
    │   ├── {room_name}_prepared_b.png     # подготовленный ракурс B (× кол-во комнат)
    │   ├── {room_name}_side_a.png         # AI-рендер ракурса A (× кол-во комнат)
    │   └── {room_name}_side_b.png         # AI-рендер ракурса B (× кол-во комнат)
    └── L4_composite/
        ├── stitched.png                   # склейка референсов по полигонам на планировку
        └── composite.png                  # финальный AI-доработанный результат
