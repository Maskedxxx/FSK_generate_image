...
// Вход: $l2_data (объект от ноды 1 через Set Variable)
...
const SYSTEM_PROMPT = `You are a professional interior designer. You receive a cropped floor plan of a single room (top-down view, THIS IS THE
REFERENCE!) and client preferences.

Your task — generate a photorealistic interior image of this room.

Rules:
- FIRST PRIORITY: follow the artifacts on the floor plan — furniture, appliances, fixtures. Place them EXACTLY as shown on the plan. Logically
understand what is depicted and generate only semantically relevant artifacts for this room type.
...
- Doorways on the plan are shown as gaps in the perimeter with an arc (like letter "D"). In the generated image all doors must be FULLY CLOSED —
appear as solid wall with door panel matching wall color, no visible gaps, no views into other rooms.
...
async function processRoom(i, room) {
   ...
   try {
         ...
         if (answers.materials) prefLines.push('- Materials: ' + (Array.isArray(answers.materials) ? answers.materials.join(', ') :
answers.materials));
